...
def data_getting(campaignID='24646526', login1='context.elama'):
    ...
    r = requests.get(url, headers={"Authorization": 'Basic cmVzZWFyY2g6ayM0enJ6UlAhKnM4'})
    try:
        d = r.json()
...
def main():
    def data_processing(login1,campaignID):
        jso = data_getting(login1, campaignID)
        if jso is None:
            with open('log.txt', 'a') as fl:
            ...
            curprice.extend([i['Bid'] if i is not None else None for i in list_dict])
            ...
            phrase_id.extend([i['Id'] if i is not None else None for i in list_dict])
            ...
            min_searchprice.extend([i.get('MinSearchPrice') if i is not None else None for i in bids_list])
            current_searchprice.extend([i.get('CurrentSearchPrice') if i is not None else None for i in bids_list])
            ...
            p11_price.extend([i[0].get('Price') if i is not None else None for i in auc_gen])
            p11_bid.extend([i[0].get('Bid') if i is not None else None for i in auc_gen])
            p12_price.extend([i[1].get('Price') if i is not None else None for i in auc_gen])
            p12_bid.extend([i[1].get('Bid') if i is not None else None for i in auc_gen])
            p13_price.extend([i[2].get('Price') if i is not None else None for i in auc_gen])
            p13_bid.extend([i[2].get('Bid') if i is not None else None for i in auc_gen])
            p21_price.extend([i[3].get('Price') if i is not None else None for i in auc_gen])
            p21_bid.extend([i[3].get('Bid') if i is not None else None for i in auc_gen])
            p22_price.extend([i[4].get('Price') if i is not None else None for i in auc_gen])
            p22_bid.extend([i[4].get('Bid') if i is not None else None for i in auc_gen])
            p23_price.extend([i[5].get('Price') if i is not None else None for i in auc_gen])
            p23_bid.extend([i[5].get('Bid') if i is not None else None for i in auc_gen])
            p24_price.extend([i[6].get('Price') if i is not None else None for i in auc_gen])
            p24_bid.extend([i[6].get('Bid') if i is not None else None for i in auc_gen])
            ...
            negative_keywords.extend([i if i else None for i in pre_neg_key_1])
            regions.extend([i.get('RegionIds') for i in ad_group_list])
    ...
    def data_frames(h):
        df_0 = DataFrame({'phrase_id': phrase_id, 'value': value,
                          ...
                          'P23_price': p23_price, 'P24_price': p24_price},
    ...
    for i in args[:-1]:
        data_processing(campaignID=i, login1=login)
    data_frames(h)
main()
